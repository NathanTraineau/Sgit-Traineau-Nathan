...
import Local_Changes.Add_Manager
import Local_Changes.Diff_Manager.get_modif_files
...
case class Relationship(new_blob_file : Option[File], former_Blob_File : Option[File],
  ...
  val list_changes: List[Change] = get_modif_files(new_blob_file_content,former_blob_File_content)
}
...
// et apreès on passe au suivant created...
    ...
    def print_diff(str: String):Unit={
      println(str)
    }
